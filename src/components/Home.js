...
export default function Home() {
  ...
  */
  ...
  const [githubStarForks, setGithubStarForks] = useState([]);
  ...
  */
  React.useEffect(() => {
    ...
    const requestOptions = {
      ...
      body: JSON.stringify({ repository: repository.key , all_repos: repositories}),
    };
    ...
    */
    fetch("/api/github", requestOptions)
      ...
      body: JSON.stringify({all_repos: repositories}),
    };
  fetch("/api/starsforks", requestOptionsStarsForks)
      .then((res) => res.json())
      .then(
        // On successful response from flask microservice
        (result) => {
    ...
    <Box sx={{ display: "flex" }}>
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
        ...
        {loading ? (
          ...
          <div>
            {/* Render linechart component for created issues for a selected repositories*/}
            <LineCharts
              title={`Open Issues for Every Repo`}
              data={githubStarForks?.issue_count}
              ylabel={'Open Issues'}
            />
            <LineCharts
            ...
            <BarCharts
              ...
              data={githubStarForks?.star_count}
              ylabel={'Stars'}
            />
            
            <BarCharts
              title={`Fork count for every repo`}
              data={githubStarForks?.fork_count}
              ylabel={'Forks'}
            />
            <StackedBarChart
              title={`Stack-Bar for created and closed issues`}
              created_issues={githubRepoData?.created}
              closed_issues={githubRepoData?.closed}
            />
